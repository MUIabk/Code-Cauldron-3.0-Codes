...
void transcendent(int tc)
{
    ...
    vec1<set<pll>> xplusy(sz(hv)), xminy(sz(hv));
    for(auto &u: hv) {
        int m = sz(adj[u]);
        FOR(j, 0, m) {
            int v = adj[u][j];
            xplusy[hvy[u]].insert({cord[v][0] + cord[v][1], v});
            xminy[hvy[u]].insert({cord[v][0] - cord[v][1], v});
    ...
    auto pull = [&] (int x) {
        if(chk(x)) {
            for(auto &v: hv) {
                if(go[hvy[x]][hvy[v]]) {
                    xplusy[hvy[v]].erase({cord[x][0] + cord[x][1], x});
                    xminy[hvy[v]].erase({cord[x][0] - cord[x][1], x});
    ...
    auto push = [&] (int x) {
        if(chk(x)) {
            for(auto &v: hv) {
                if(go[hvy[x]][hvy[v]]) {
                    xplusy[hvy[v]].insert({cord[x][0] + cord[x][1], x});
                    xminy[hvy[v]].insert({cord[x][0] - cord[x][1], x});
                    if(go[hvy[x]][hvy[v]] == 1) op[hvy[v]] += cord[x][0] + cord[x][1];
                    else op[hvy[v]] -= cord[x][0] + cord[x][1];
                }
            }
    ...
    auto calc = [&] (int x) {
        pll ans = {0, x};
        if(chk(x)) {
            if(xplusy[hvy[x]].empty()) return ans.ss + 1;
            auto tmnplus = *xplusy[hvy[x]].begin();
            auto mnplus = *--xplusy[hvy[x]].ub({tmnplus.ff, INF});
            auto mxplus = *xplusy[hvy[x]].rbegin();
            auto tmnmin = *xminy[hvy[x]].begin();
            auto mnmin = *--xminy[hvy[x]].ub({tmnmin.ff, INF});
            auto mxmin = *xminy[hvy[x]].rbegin();
            ckmax(ans, {abs(cord[x][0] + cord[x][1] - mnplus.ff), mnplus.ss});
            ckmax(ans, {abs(cord[x][0] + cord[x][1] - mxplus.ff), mxplus.ss});
            ckmax(ans, {abs(cord[x][0] - cord[x][1] - mnmin.ff), mnmin.ss});
            ckmax(ans, {abs(cord[x][0] - cord[x][1] - mxmin.ff), mxmin.ss});
        } else {
            for(auto &v: adj[x]) {
                ckmax(ans, {abs(cord[x][0] - cord[v][0]) + abs(cord[x][1] - cord[v][1]), v});
            } 
        } return ans.ss + 1;
    };
    int q; cin >> q;
